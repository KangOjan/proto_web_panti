...
export default function ProgramManagement(props) {
  const simk = useSimk();
  const priorityPrograms = (props.priorityPrograms && props.priorityPrograms.length > 0) ? props.priorityPrograms : (props.campaigns || simk.campaigns);
  const onSaveProgram = props.onSaveProgram || props.onSaveCampaign || simk.handleSaveCampaign;
  const onDeleteProgram = props.onDeleteProgram || props.onDeleteCampaign || simk.handleDeleteCampaign;
  ...
  return (
    <MainLayout currentRoute="program">
          ...
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in no-print">
            <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-100 text-center space-y-4">
              ...
              <div className="flex items-center space-x-2 pt-2">
                ...
                <button
                  onClick={() => {
                    if (onDeleteProgram) onDeleteProgram(deleteConfirmId);

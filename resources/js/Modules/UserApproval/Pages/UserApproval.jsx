...
export default function UserApproval(props) {
  const simk = useSimk();
  const users = (props.users && props.users.length > 0) ? props.users : simk.users;
  const onApproveUser = props.onApproveUser || simk.handleApproveUser;
  const onRejectUser = props.onRejectUser || simk.handleRejectUser;
  ...
  return (
    <MainLayout currentRoute="approval">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-indigo-500 font-medium relative"
            />
          </div>
        </div>
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-slate-500 text-[11px]">
                        {user.registeredAt ? new Date(user.registeredAt).toLocaleDateString('id-ID', {
                          day: 'numeric',
                          month: 'short',
                          year: 'numeric'
                        }) : '-'}

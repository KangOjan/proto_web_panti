// PSAK 45 / ISAK 35 Non-Profit Financial Report Component with PDF Print trigger
...
// Filter transactions based on period
  ...
  // Expense breakdowns
  const expenseOperasional = reportTransactions
    .filter(t => t.type === 'pengeluaran' && t.category === 'Operasional')
    .reduce((sum, t) => sum + t.amount, 0);

  const expenseSPP = reportTransactions
    .filter(t => t.type === 'pengeluaran' && t.category === 'SPP/Pendidikan')
    .reduce((sum, t) => sum + t.amount, 0);

  const expenseKonsumsi = reportTransactions
    .filter(t => t.type === 'pengeluaran' && t.category === 'Konsumsi')
    .reduce((sum, t) => sum + t.amount, 0);

  const expenseKesehatan = reportTransactions
    .filter(t => t.type === 'pengeluaran' && t.category === 'Kesehatan')
    .reduce((sum, t) => sum + t.amount, 0);

  const expenseLainnya = reportTransactions
  ...
  // Calculate opening balance before selected period
  ...
  const saldoAwalKas = priorIncome - priorExpense + 15000000; // Base starting balance
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in space-y-6">
      ...
      {/* Top Filter & Print Action Bar (Hidden on Print) */}
      ...
      <div className="bg-white rounded-3xl p-8 shadow-md border border-slate-200 printable-area text-slate-900 space-y-6">
        ...
        {/* Section 1: Pendapatan & Sumber Dana */}
        <div>
        ...
        {/* Section 2: Beban Operasional & Program */}
        <div>
        ...
        {/* Section 3: Perubahan Aset Netto & Saldo Kas */}
...
window.FinancialReportPSAK45 = FinancialReportPSAK45;
